...
async def async_scrape_inmate_data(session: aiohttp.ClientSession, details_path: str) -> Optional[Dict]:
    """
    ...
    """
    try:
        async with session.get(details_path, timeout=aiohttp.ClientTimeout(total=30)) as response:
            if response.status != 200:
                logger.warning(f"Failed to fetch {details_path}: HTTP {response.status}")
                return None
...
async def async_image_url_to_base64(session: aiohttp.ClientSession, image_url: str) -> Optional[str]:
    """
    ...
    try:
        async with session.get(image_url, timeout=aiohttp.ClientTimeout(total=30)) as response:
            if response.status == 200:
                ...
                logger.warning(f"Failed to fetch image {image_url}: HTTP {response.status}")
                return None
    except Exception as e:
...
async def async_scrape_all_inmate_details(detail_urls: List[str], max_concurrent: int = 10) -> List[Optional[Dict]]:
    """
...
def scrape_with_threading(detail_urls: List[str], max_workers: int = 5) -> List[Optional[Dict]]:
    """
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_index):
            ...
            try:
                ...
                logger.error(f"Threading scrape failed for URL {detail_urls[index]}: {e}")
                results[index] = None
...
def scrape_washington_so_ar_optimized(
    ...
    if use_async and len(detail_urls) > 0:
        try:
            logger.info(f"Using async scraping with {max_concurrent} concurrent requests")
            detailed_data = asyncio.run(async_scrape_all_inmate_details(detail_urls, max_concurrent))
        except Exception as e:
            logger.error(f"Async scraping failed, falling back to threading: {e}")
            detailed_data = scrape_with_threading(detail_urls, max_workers=min(5, len(detail_urls)))
    else:
    ...
    for basic_data, details in zip(basic_inmate_data, detailed_data):
        if details is None:
            logger.warning(f"Skipping inmate {basic_data['name']} due to failed detail scraping")
            continue
        ...
        logger.debug(f"Added inmate: {basic_data['name']} with arrest date: {basic_data['arrest_date']}")
    
    end_time = time.time()
    scraping_time = end_time - start_time
    
    logger.success(f"Successfully scraped {successful_scrapes}/{len(basic_inmate_data)} inmates in {scraping_time:.2f} seconds")
    logger.info(f"Average time per inmate: {scraping_time/len(basic_inmate_data):.2f} seconds")
    ...
    logger.info(f"Total processing time: {total_time:.2f} seconds (scraping: {scraping_time:.2f}s, processing: {process_time:.2f}s)")
